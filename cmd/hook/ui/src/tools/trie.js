 ...
 */
export default function Trie(options, pathKeys) {
	this._kids = new Map
	this._leaf = false
	this._values = []
	this._path = pathKeys || []
...
Trie.prototype = {
	...
	deleteById : function(key, id) {
		...
		if (node) {
			node._values = node._values.filter(item => item.id !== id)
			return true
		}
		return false
	}
...
function delLeaf(tri, keys) {
	...
	return (!kid._leaf && !kid._kids.size && kid.delete(key)) || res
}
